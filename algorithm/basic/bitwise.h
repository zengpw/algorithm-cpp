...
int findNumberAppearsOnce2(int array[], int length)
{
    ...
    // 将这个数组变回十进制，如果第 k 位为 1, 则乘以 2 的 k 次方，最后相加即可
    int rtn = 0;
    for (int k = 0; k < 32; ++k)
    {
        if (result[k] == 1)
...
//
//                  000 -> 0, 011 -> 0, 001 -> 1, 111 -> 1
//
//      如果该位异或结果为 1, 那么按位遍历所有的数字，把 001, 111 按照 0, 1 分别分组
//
//                  如果 1 是奇数，并且 0 那组异或结果为 0 (111), 此时求不出结果，因为 3 个数字在一组
//                  如果 1 是奇数，并且剩下那组异或结果不为 0 (001), 那么这个 1 数字就可以挑出来了
//
//                      可以直接一份，放到数组最后，将数组变为 2 个不同数字，变成 findNumberAppearsOnce 题型
//
